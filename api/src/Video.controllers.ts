...
export const newVideo: RequestHandler = async (req, res) => {
...
export const deleteVideo: RequestHandler = async (req, res) => {
  const videoInfo = await Video.findByIdAndDelete(req.params.id);
  if (!videoInfo) {
    return res.status(204).json();
  }
  return res.json({ message: 'Video deleted', videoInfo });
};
